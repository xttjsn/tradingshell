...
def ordering_logic(context, data):
    vix  = data.current('VIX','price')
    context.vix_h.append(vix)
    vxst = data.current('VXST','price')
    vxv  = data.current('VXV','price')
    vx1  = data.current('v1','Settle')
    vx2  = data.current('v2','Settle')
    
    vix_p = context.vixpipe.loc[symbol('VXX')]['vix']
    
    if  vix <> vix_p: log.info('fetcher and pipeline do not agree')
    # Volatility terms 9 days (VXST), 30 days (VIX), 93 days (VXV)
    ...
    if ivts_median < (np.mean(context.ivts_medianfiltered[-90:]) -0.01):
        rebalance(context.sidsLongSPY , data, 0.49) 
        rebalance(context.sidsShortSPY, data, 0.0) 
    elif ivts_median > (np.mean(context.ivts_medianfiltered[-90:]) +0.01):
...
class Get_VIX(CustomFactor):    
    ...
    def compute(self, today, assets, out, vix):
        vix_values = pd.DataFrame(vix, columns=assets)[sid(38054)]
        out[:] = vix_values[0] * np.ones(len(assets))      
